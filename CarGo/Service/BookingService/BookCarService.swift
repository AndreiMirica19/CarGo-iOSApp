...
struct BookCarService: BookCarProtocol {
    static func bookCar(bookCarData: BookCarData) async throws -> (Data?, NetworkError?) {
        
        guard let url = URL(string: ApiEndpoint.url + ApiEndpoint.bookCar.rawValue) else {
        ...
        do {
            let jsonData = try JSONEncoder().encode(bookCarData)
